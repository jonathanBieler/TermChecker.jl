...
function checkpkg(pkg::String, term::Term)
...
#                sleep(0.01)
                ...
                n_standard += sum( file_symbols .== term.standard_name )
                for i = 1:length(n_alt)
                    n_alt[i] += sum( file_symbols .== term.alternative_names[i] )
                end
            end
        end
    end
    
    println( string(term.standard_name) * ": " * string(n_standard) )
    for i = 1:length(n_alt)
        println( string(term.alternative_names[i]) * ": " * string(n_alt[i]) )
    end
